  ...
  return (
    <SiteLayout>
      ...
      {devlogEntries.length > 0 && (
        <section className="container py-20">
          <p className="text-primary font-semibold uppercase tracking-widest text-xs mb-3 text-center">Development Stories</p>
          <h2 className="font-display text-4xl md:text-5xl text-center mb-12">Our <span className="text-gold">Devlog</span></h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {devlogEntries.map((entry) => (
              <div
                key={entry.id}
                onClick={() => navigate(`/devlog/${entry.id}`)}
                className="group cursor-pointer h-full"
              >
                <div className="relative h-full bg-card border border-border rounded-sm overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-lg -rotate-1 group-hover:rotate-0 transform"
                  style={{
                    boxShadow: "0 4px 12px rgba(0, 0, 0, 0.3), 0 0px 0 8px rgba(0, 0, 0, 0.05)"
                  }}
                >
                  {/* Image Section */}
                  {entry.image && (
                    <div className="w-full h-48 md:h-56 overflow-hidden bg-secondary">
                      <img
                        src={entry.image}
                        alt={entry.title}
                  ...
                  {/* Content Section - Polaroid Style Bottom */}
                  <div className="p-5 space-y-3" style={{ paddingBottom: "1.5rem" }}>
                    <div>
                      <time className="text-xs text-muted-foreground">{entry.date}</time>
                      <h3 className="font-display text-lg font-bold mt-2 line-clamp-2 group-hover:text-primary transition-colors">
                        {entry.title}
                      </h3>
                    </div>
